...
async function queryASTToSqlAST(resolveInfo, options, context) {
  ...
  await populateASTNode.call(resolveInfo, queryAST, parentType, sqlAST, namespace, 0, options, context);
...
async function populateASTNode(queryASTNode, parentTypeNode, sqlASTNode, namespace, depth, options, context) {
  var _ref5;
  ...
  if (!fieldConfig.ignoreTable && TABLE_TYPES.includes(gqlType.constructor.name) && config.sqlTable) {
    ...
    await handleTable.call(this, sqlASTNode, queryASTNode, field, gqlType, namespace, grabMany, depth, options, context);
...
async function handleTable(sqlASTNode, queryASTNode, field, gqlType, namespace, grabMany, depth, options, context) {
  const config = (0, _util.getConfigFromSchemaObject)(gqlType);
  const fieldConfig = (0, _util.getConfigFromSchemaObject)(field);
  sqlASTNode.type = 'table';
  const sqlTable = await (0, _util.asyncUnthunk)(config.sqlTable, sqlASTNode.args || {}, context);
  ...
  if (fieldConfig.sqlJoin) {
    ...
    const junctionTable = await (0, _util.asyncUnthunk)((0, _util.ensure)(fieldConfig.junction, 'sqlTable'), sqlASTNode.args || {}, context);
  ...
  if (queryASTNode.selectionSet) {
    if (gqlType.constructor.name === 'GraphQLUnionType' || gqlType.constructor.name === 'GraphQLInterfaceType') {
      ...
      await handleUnionSelections.call(this, sqlASTNode, children, queryASTNode.selectionSet.selections, gqlType, namespace, depth, options, context);
    } else {
      await handleSelections.call(this, sqlASTNode, children, queryASTNode.selectionSet.selections, gqlType, namespace, depth, options, context);
    }
  }
}

async function handleUnionSelections(sqlASTNode, children, selections, gqlType, namespace, depth, options, context, internalOptions = {}) {
  for (let selection of selections) {
    switch (selection.kind) {
      case 'Field':
        ...
        await populateASTNode.call(this, selection, gqlType, newNode, namespace, depth + 1, options, context);
        break;
      ...
      case 'InlineFragment':
        {
          ...
          const handler = deferToObjectType ? await handleSelections : await handleUnionSelections;
...
async function handleSelections(sqlASTNode, children, selections, gqlType, namespace, depth, options, context, internalOptions = {}) {
  for (let selection of selections) {
    switch (selection.kind) {
      case 'Field':
        const existingNode = children.find(child => child.fieldName === selection.name.value && child.type === 'table');
      ...
      case 'InlineFragment':
        {
          ...
          if (sameType || interfaceType) {
            await handleSelections.call(this, sqlASTNode, children, selection.selectionSet.selections, gqlType, namespace, depth, options, context, internalOptions);
          }
        }
        break;
      ...
      case 'FragmentSpread':
        {
          ...
          if (sameType || interfaceType) {
            await handleSelections.call(this, sqlASTNode, children, fragment.selectionSet.selections, gqlType, namespace, depth, options, context, internalOptions);
          }
        }
        break;
